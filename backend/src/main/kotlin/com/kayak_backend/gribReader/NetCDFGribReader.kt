...
class NetCDFGribReader : GribReader {
    ...
    private fun findLatLon(
        ...
        if (lat < latData.getDouble(0) || lat > latData.getDouble(latData.shape[0] - 1)) throw GribIndexError("Latitude out of bounds " + lat)
        ...
        if (lon < lonData.getDouble(0) || lon > lonData.getDouble(lonData.shape[0] - 1)) throw GribIndexError("Longitude out of bounds " + lon)
